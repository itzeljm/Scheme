...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError('number of argumments and values is not equal')
...
class MuProcedure(LambdaProcedure):
    ...
    def apply(self, args, env):
        ...
        if proper_tail_recursion:
            # Implemented in Question 22.
            "*** YOUR CODE HERE ***"
            new_F = env.make_call_frame(self.formals, args)
            return (self.body, new_F)
        else:
            "*** YOUR CODE HERE ***"
            new_F = env.make_call_frame(self.formals, args)
            return (scheme_eval(self.body,new_F), None)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, function[0])
...
def do_let_form(vals, env):
    ...
    for param in bindings:
        ...
        names = Pair(param.first, names)                    ######
        values = Pair(scheme_eval(param[1], env), values)
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError("formals have wrong format %f", formals)
    else:
        unique_formals = []
        for symbol in formals:
            if not scheme_symbolp(symbol):
                raise SchemeError("one or more values are not scheme formals %s", formals)
            if symbol in unique_formals:
                raise SchemeError("formals require unique symbols")
            else:
                unique_formals.append(symbol)
